...
parser.add_argument("--env_name", default="carla-v1")  # OpenAI gym environment name， BipedalWalker-v2
...
class TD3():
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.scheduler_policy = lr_scheduler.CosineAnnealingLR(self.actor_optimizer, 10000, eta_min=0.0005, last_epoch=-1)
        self.scheduler_critic1 = lr_scheduler.CosineAnnealingLR(self.critic_1_optimizer, 10000, eta_min=0.0005, last_epoch=-1)
        self.scheduler_critic2 = lr_scheduler.CosineAnnealingLR(self.critic_2_optimizer, 10000, eta_min=0.0005, last_epoch=-1)
    ...
    if args.mode == 'test':
        ...
        for i in range(args.num_iteration):
            ...
            for t in range(2000):
                ...
                next_state, reward, done, collision_ = env.step(action)
                ...
                state_for_dataset.append(state)
                next_state_for_dataset.append(next_state)
                action_for_dataset.append(action)
                reward_for_dataset.append(reward)
                done_for_dataset.append(done)
                if i+1 % 10 == 0:
            ...
            print("*******************************************************************************************")
            if i > 20:
                agent.writer.add_scalar("reward", ep_r, frame)
            ep_r = 0
            if i % 100 == 0:
                agent.writer.add_scalar("Suceess rate", success / 100, frame)
                agent.writer.add_scalar("Collision Rate", collision / 100, frame)
                success = 0
                collision = 0
        ...
        dataset['next_state'] = next_state_for_dataset
        dataset['action'] = action_for_dataset
        dataset['reward'] = reward_for_dataset
        dataset['done'] = done_for_dataset
        if args.save_dataset:
            pickle.dump(dataset, open('dataset_from_td3', 'wb'))
