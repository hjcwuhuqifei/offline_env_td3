...
class ReplayBuffer(object):
    ...
    def store_transition(self, obs_n, a_n, r_n, obs_next_n, done_n):
        for agent_id in range(self.N):
            self.buffer_obs_n[agent_id][self.count] = obs_n[0]
            self.buffer_a_n[agent_id][self.count] = a_n[0]
            self.buffer_r_n[agent_id][self.count] = r_n
            self.buffer_s_next_n[agent_id][self.count] = obs_next_n
